...
function checkMemberAuth(req, res, next) {
    ...
    if (!req.session.user.member_id) {
        return res.status(400).send(
            ...
            '<p>Your account is not linked to a member profile. Contact admin.</p>' +
...
function blockMemberAccess(req, res, next) {
    if (req.session && req.session.user && req.session.user.role === 'member') {
        return res.status(403).send(
            '<h2>Access Denied</h2>' +
            '<p>You do not have permission to view this page.</p>' +
            '<a href="/member/dashboard">Go to your dashboard</a>'
        );
    }
    next();
}
...
// ==================== DASHBOARD ====================
  app.get('/', checkAuth, blockMemberAccess, asyncHandler(async (req, res) => {
  const db = dbConfig;
...
pageRoutes.forEach(page => {
    app.get(`/${page}`, checkAuth, blockMemberAccess, (req, res) => {
...
app.get('/members/:id/edit', checkAuth, blockMemberAccess, asyncHandler(async (req, res) => {
  ...
  const [rows] = await db.execute(`
    SELECT 
      ...
      occupation, village_lc1, Status, parish, sub_county, place_of_birth, district,
...
app.post('/members/:id/edit', checkAuth, upload.single('photo'), asyncHandler(async (req, res) => {
  ...
  const memberData = {
    ...
    Status: (req.body.status && req.body.status.trim()) || null,
    ...
    father_status: (req.body.father_status && req.body.father_status.trim()) || null,
    ...
    mother_status: (req.body.mother_status && req.body.mother_status.trim()) || null,
  ...
  try {
    ...
    await db.execute(`
      ...
      SET First_name=?, Middle_Name=?, Last_Name=?, tel_no=?, sex=?, marital_status=?, 
          ...
          parish=?, sub_county=?, place_of_birth=?, district=?,
          next_of_kin_First_name=?, next_of_kin_Last_name=?, next_of_kin_tel=?, next_of_kin_address=?,
    ...
    `, [
      ...
      memberData.parish, memberData.sub_county, memberData.place_of_birth, memberData.district,
...
async function buildNotificationsForRequest(req, limit) {
    ...
    try {
        const [intRows] = await dbConfig.execute(`
            SELECT loan_id, MAX(calculation_date) AS last_date,
                   SUM(daily_interest_amount) AS total_today,
                   COUNT(*) AS loans_processed
            FROM loan_daily_interest
            WHERE calculation_date = (SELECT MAX(calculation_date) FROM loan_daily_interest)
        ...
        if (intRows.length) {
            ...
            notifications.push({
                ...
                message:    'Daily interest calculated for ' + intRows.length + ' loan(s) on ' + r.last_date,
                created_at: r.last_date,
                read:       true,
                status:     'done'
            });
        }
    ...
    try {
        ...
        overdueRows.forEach(function(r) {
            notifications.push({
                ...
                type:       'overdue',
                title:      'Overdue Payment',
                message:    r.member_name + ' — UGX ' + Number(r.expected_payment).toLocaleString() + ' due ' + (r.due_date ? String(r.due_date).split('T')[0] : ''),
                created_at: r.due_date,
                read:       false,
                status:     null
            });
    ...
    try {
        const [memberRows] = await dbConfig.execute(`
    ...
    try {
        const [loanRows] = await dbConfig.execute(`
            ...
            FROM loans l
            JOIN members_mst m ON l.member_id = m.id
            WHERE l.disbursement_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
            ORDER BY l.disbursement_date DESC
            LIMIT 5
        `);
        loanRows.forEach(function(r) {
            notifications.push({
                ...
                message:    r.member_name + ' — UGX ' + Number(r.loan_amount).toLocaleString(),
                created_at: r.created_at,
                read:       true,
                status:     null
            });
    ...
    /* 5. Recent loan repayments (last 7 days) — uses loan_payment_history */
    try {
        const [repayRows] = await dbConfig.execute(`
            SELECT lph.id, lph.amount, lph.payment_date AS created_at,
                   CONCAT(m.First_name,' ',m.Last_Name) AS member_name
            ...
            JOIN loans l ON lph.loan_id = l.id
            JOIN members_mst m ON l.member_id = m.id
            WHERE lph.payment_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
            ORDER BY lph.payment_date DESC
            LIMIT 5
        `);
        repayRows.forEach(function(r) {
    ...
    try {
        const [txnRows] = await dbConfig.execute(`
            SELECT t.id, t.Amount AS amount, t.transaction_type, t.created_at,
                   CONCAT(m.First_name,' ',m.Last_Name) AS member_name
            FROM transactions t
            JOIN members_mst m ON t.member_id = m.id
...
//==============MEMBERS DASHBOARD===========================
app.get('/member/dashboard', checkMemberAuth, asyncHandler(async (req, res) => {
    const db       = dbConfig;
    ...
    let loanTotalPaid=0, loanRepaidPct=0, overdueCount=0, nextPayment=null, loanSchedule=[];
    if (activeLoan) {
        ...
        const [npRows] = await db.execute(`
            SELECT payment_number, DATE_FORMAT(due_date,'%d %b %Y') AS due_date, expected_payment
            FROM loan_schedule WHERE loan_id=? AND status IN ('Pending','Overdue')
            ORDER BY payment_number ASC LIMIT 1
        `, [activeLoan.id]);
        nextPayment = npRows[0] || null;

        const [schRows] = await db.execute(`
            SELECT payment_number, DATE_FORMAT(due_date,'%d %b %Y') AS due_date,
                   expected_payment, expected_principal, expected_interest,
                   COALESCE(paid_amount,0) AS paid_amount, status
            FROM loan_schedule WHERE loan_id=? ORDER BY payment_number DESC LIMIT 12
        `, [activeLoan.id]);
        loanSchedule = schRows.reverse();
    }

    const [monthlyRows] = await db.execute(`
        SELECT MONTH(tran_date) AS m, COALESCE(SUM(Amount),0) AS v
        FROM transactions WHERE member_id=? AND transaction_type='Saving'
          AND YEAR(tran_date)=YEAR(CURDATE())
        GROUP BY MONTH(tran_date)
    `, [memberId]);
    ...
    let run=0;
    const cumulativeSavingsArr = monthlySavingsArr.map(v => (run += v));
    ...
    const [txnRows] = await db.execute(`
        ...
        FROM transactions WHERE member_id=? ORDER BY created_at DESC LIMIT 10
    `, [memberId]);

    const [actRows] = await db.execute(`
        SELECT transaction_type, Amount, Debit_Credit,
               DATE_FORMAT(created_at,'%d %b %Y %H:%i') AS created_at
        FROM transactions WHERE member_id=? ORDER BY created_at DESC LIMIT 5
    `, [memberId]);

    res.render('member_dashboard', {
        currentPage:'member_dashboard', user:req.session.user, member,
...
/* ── Insurance cover ── */
app.get('/member/insurance', checkMemberAuth, asyncHandler(async (req, res) => {
    const db = dbConfig;
    const memberId = req.session.user.member_id;
    const [rows] = await db.execute(`
        SELECT DATE_FORMAT(tran_date,'%d-%b-%Y') AS tran_date,
    ...
    res.render('member_insurance', {
        currentPage:'member_insurance', user:req.session.user,
        transactions:rows, total, pct: Math.min(Math.round((total/60000)*100),100)
    });
}));


/* ── Welfare fee ── */
app.get('/member/welfare', checkMemberAuth, asyncHandler(async (req, res) => {
    const db = dbConfig;
    const memberId = req.session.user.member_id;
    const [rows] = await db.execute(`
        SELECT DATE_FORMAT(tran_date,'%d-%b-%Y') AS tran_date,
    ...
    `, [memberId]);
    const total = rows.reduce((s,r) => s + parseFloat(r.Amount), 0);
    res.render('member_welfare', {
        currentPage:'member_welfare', user:req.session.user,
        transactions:rows, total, pct: Math.min(Math.round((total/240000)*100),100)
    });
}));


/* ── Membership fee ── */
app.get('/member/membership', checkMemberAuth, asyncHandler(async (req, res) => {
    const db = dbConfig;
    const memberId = req.session.user.member_id;
    const [rows] = await db.execute(`
        SELECT DATE_FORMAT(tran_date,'%d-%b-%Y') AS tran_date,
               Amount, payment_period, description
        FROM transactions WHERE member_id=? AND transaction_type='Membership Fee'
        ORDER BY tran_date DESC
    `, [memberId]);
    const total = rows.reduce((s,r) => s + parseFloat(r.Amount), 0);
...
app.get('/member/biodata', checkMemberAuth, asyncHandler(async (req, res) => {
    ...
    /* ── Full member record with lifetime totals ── */
    const [rows] = await db.execute(`
        SELECT
            a.*,
            DATE_FORMAT(a.Date_of_Birth, '%d-%b-%Y') AS Date_of_Birth,
    ...
    if (!rows.length) return res.status(404).send('Member record not found.');
    const member = rows[0];
    ...
    /* ── Annual financial summary (current year) ── */
    const [finances] = await db.execute(`
        SELECT
            a.id AS member_id,
            YEAR(CURDATE()) AS payment_year,
            ...
            COALESCE(t.year_total_loan_repayment,   0) AS year_total_loan_repayment,
            COALESCE(
                (t.year_total_loan_repayment / NULLIF(t.year_total_loan_received,0)) * 100,
                0
            ) AS year_percentage_loan_repayment,
            COALESCE((t.year_total_insurance_cover / 60000)  * 100, 0) AS year_percentage_insurance_payment,
            COALESCE((t.year_total_welfare_fee     / 240000) * 100, 0) AS year_percentage_welfare_fee
        FROM members_mst a
        LEFT JOIN (
            SELECT
                ...
                SUM(CASE WHEN Debit_Credit='Credit' AND YEAR(tran_date)=YEAR(CURDATE())
                         ...
                         THEN Amount ELSE 0 END) AS year_total_welfare_fee,
                SUM(CASE WHEN Debit_Credit='Credit'
                         AND transaction_type='Loan Disbursement'
                         THEN Amount ELSE 0 END) AS year_total_loan_received,
                SUM(CASE WHEN Debit_Credit='Debit'
                         AND transaction_type='Loan Repayment'
                         THEN Amount ELSE 0 END) AS year_total_loan_repayment
            FROM transactions
            GROUP BY member_id
        ) t ON a.id = t.member_id
        WHERE a.id = ?
    ...
    const finance = finances[0] || {
        ...
        year_percentage_loan_repayment:      0,
        year_percentage_insurance_payment:   0,
        year_percentage_welfare_fee:         0
    };
